...
#define ALLY_RESP_CURVE_SHOW(_name, _point_n)                                                     \
	...
	{                                                                                         \
		...
		return sysfs_emit(                                                                \
			buf, "%d %d\n",                                                           \
			ally_cfg->response_curve[ally_cfg->mode][btn_pair_side_left][idx],        \
			ally_cfg->response_curve[ally_cfg->mode][btn_pair_side_right][idx + 1]);  \
	}
...
#define ALLY_RESP_CURVE_STORE(_name, _side, _point_n)                                              \
	...
	{                                                                                          \
		int ret =                                                                          \
			__gamepad_store_response_curve(dev, buf, btn_pair_side_##_side, _point_n); \
		if (ret < 0)                                                                       \
			return ret;                                                                \
		return count;                                                                      \
	}
...
#define ALLY_AXIS_DEADZONE_SHOW(_axis)                                                          \
	static ssize_t _axis##_deadzone_show(struct device *dev, struct device_attribute *attr, \
					     char *buf)                                         \
	{                                                                                       \
		struct ally_gamepad_cfg *ally_cfg = drvdata.gamepad_cfg;                        \
		int side, is_tr;                                                                \
...
#define ALLY_AXIS_DEADZONE_STORE(_axis)                                                          \
	static ssize_t _axis##_deadzone_store(struct device *dev, struct device_attribute *attr, \
					      const char *buf, size_t count)                     \
	{                                                                                        \
		struct ally_gamepad_cfg *ally_cfg = drvdata.gamepad_cfg;                         \
		if (!drvdata.gamepad_cfg)                                                        \
...
#define ALLY_BTN_SHOW(_fname, _pair, _side, _secondary)                                            \
	...
	{                                                                                          \
		...
		return sysfs_emit(buf, "%s\n",                                                     \
				  __btn_map_to_string(ally_cfg, _pair, _side, _secondary));        \
	}

#define ALLY_BTN_STORE(_fname, _pair, _side, _secondary)                                    \
	static ssize_t _fname##_store(struct device *dev, struct device_attribute *attr,    \
				      const char *buf, size_t count)                        \
	{                                                                                   \
		struct ally_gamepad_cfg *ally_cfg = drvdata.gamepad_cfg;                    \
		if (!drvdata.gamepad_cfg)                                                   \
			return -ENODEV;                                                     \
		int ret = __gamepad_mapping_store(ally_cfg, buf, _pair, _side, _secondary); \
		if (ret < 0)                                                                \
			return ret;                                                         \
		return count;                                                               \
	}
...
#define ALLY_BTN_TURBO_SHOW(_fname, _pair, _side)                                             \
	static ssize_t _fname##_turbo_show(struct device *dev, struct device_attribute *attr, \
					   char *buf)                                         \
	{                                                                                     \
...
#define ALLY_BTN_TURBO_STORE(_fname, _pair, _side)                                             \
	static ssize_t _fname##_turbo_store(struct device *dev, struct device_attribute *attr, \
					    const char *buf, size_t count)                     \
	{                                                                                      \
