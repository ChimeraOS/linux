...
static int asus_wmi_add(struct platform_device *pdev)
{
	...
	/*
	 * Ensure non-updated Ally devices start in good state. Fixes reboot issues.
	 * Safe to call when MCU FW is fixed version as it is ignored if state is good.
	 */
...
{
	/* Fixed MCU FW is not affected but do this in-case FW is not fixed version */
	if (ally_mcu_usb_switch) {
